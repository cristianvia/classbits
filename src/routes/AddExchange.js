...
let arrayClassroom = JSON.parse(localStorage.getItem("exchange") || []);
...
function SaveDataToLocalStorage(dataFromState) {
    ...
    a = JSON.parse(localStorage.getItem('exchange')) || [];
    // Push the new data (whether it be an object or anything else) onto the array
    a.push(dataFromState);
    // Alert the array value
    // Re-serialize the array back into a string and store it in localStorage
    localStorage.setItem('exchange', JSON.stringify(a));
}
...
const AddExchange = () => {
    ...
    useEffect(() => {
        ...
        const handleForm = async (event) => {
            ...
            alert("Bescanvi afegit correctament")
    ...
    return (
        <div>
            <Header />
            <h2 style={{ textAlign: "center" }}>Afegir bescanvis</h2>
